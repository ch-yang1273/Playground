...
- 참고 MDN 교차 출처 리소스 공유 [자격 증명을 포함한 요청](https://developer.mozilla.org/ko/docs/Web/HTTP/CORS#%EC%9E%90%EA%B2%A9_%EC%A6%9D%EB%AA%85%EC%9D%84_%ED%8F%AC%ED%95%A8%ED%95%9C_%EC%9A%94%EC%B2%AD)
...
`http://test-cors.org` 페이지에서는 테스트가 정상적으로 되지 않으니 프로토콜(https)를 확인!

CORS 테스트의 요청과 응답은 `http://test-cors.org` 페이지에서 개발자 모드(F12)로 확인한다.

### 1. https://test-cors.org Client 페이지에서 Request 설정

Mapping 될 Remote URL을 설정하고, Custom Header도 테스트하기 위해 `key: value` 헤더도 추가했다.

![test-cors](https://user-images.githubusercontent.com/61798028/257121177-f1c3f641-0d7f-4a0a-bfd8-148321cf2cb3.png)

### 2. CORS 정책 설정 없이 테스트

CORS 정책을 따로 설정하지 않고, 다른 출처에 대한 리소스를 요청하여 CORS 정책을 위반하는 테스트를 했다.
...
위에서 설정한 것과 같이 모든 CORS 정책을 와일드 카드로 풀고 테스트를 진행했다. Credentials는 허용하지 않았다.
